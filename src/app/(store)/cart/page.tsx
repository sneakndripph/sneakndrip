                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1.5">
                        <select
                          ...
                          onChange={e => {
                            ...
                            if (result) toastSuccess(`Size updated to ${newSize}`);
                          }}
                            ...
                            {(["full_payment", "downpayment"] as const).map(pt => (
                              <button key={pt} type="button"
                                onClick={() => {
                                  ...
                                  applyMergeResult(item.id, result);
                                  if (result) toastSuccess("Payment type updated");
                                }}
        ...
        onConfirm={() => {
          if (removingItem) {
            ...
            toastSuccess("Item removed from cart", {
              subtitle: "Click undo to restore",
              duration: 6000,
              action: {
                label: "Undo",
                onClick: () => addItem(snapshot.product, snapshot.size, snapshot.payment_type, snapshot.quantity),
              },
            });
          }
