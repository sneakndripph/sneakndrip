...
import { ChevronLeft, ShoppingBag, Zap, Shield, Truck, Clock } from "lucide-react";
import toast from "react-hot-toast";

export default function ProductPage({ params }: { params: { slug: string } }) {
  const found = MOCK_PRODUCTS.find(p => p.slug === params.slug);
  if (!found) notFound();
  const product = found;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div>
            {/* Main image */}
            <div
              className="relative aspect-square mb-3 rounded-xl overflow-hidden flex items-center justify-center"
            ...
            >
              ...
              <div className="absolute top-4 left-4">
                {isPreOrder ? (
                  <span className="text-xs font-black uppercase px-3 py-1.5 text-white" style={{ background: BRAND.red }}>
                    Pre-Order
                  </span>
                ) : (
                  <span className="text-xs font-black uppercase px-3 py-1.5 text-white" style={{ background: BRAND.teal }}>
                    On Hand
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {images.map((bg, i) => (
                <button key={i} onClick={() => setImageIdx(i)}
                  className="aspect-square rounded-lg overflow-hidden flex items-center justify-center transition-all"
                  style={{
                    background: bg,
                    border: `2px solid ${imageIdx === i ? BRAND.teal : BRAND.border}`,
                  }}>
          ...
          <div>
            ...
            {product.colorway && (
              <p className="text-sm mb-4" style={{ color: BRAND.muted }}>{product.colorway}</p>
            )}
            ...
            <div className="p-5 rounded-xl mb-6" style={{ background: BRAND.card, border: `1px solid ${BRAND.cardBorder}` }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="text-3xl font-black" style={{ fontFamily: FONTS.display, fontSize: "2.5rem", color: BRAND.black, letterSpacing: "0.02em" }}>
              ...
              <div className="grid grid-cols-2 gap-2 mb-1">
                {[
                  ...
                  <button key={opt.value} onClick={() => setPaymentType(opt.value)}
                    ...
                    <p className="text-sm font-black mt-0.5" style={{ fontFamily: FONTS.display, fontSize: "1.1rem" }}>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                {selectedSize && (
                  <p className="text-sm font-semibold" style={{ color: BRAND.teal }}>{selectedSize}</p>
                )}
              </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: `1.5px solid ${isSelected ? BRAND.black : outOfStock ? BRAND.border : BRAND.border}`,
              ...
              <p className="text-xs mt-2" style={{ color: BRAND.muted }}>
                Numbers in red indicate limited stock remaining.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 mb-8">
              ...
              <button
                onClick={handleAddToCart}
                className="w-full py-4 font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-opacity hover:opacity-90"
                style={{ background: BRAND.teal, color: "#fff", fontFamily: FONTS.body }}
              >
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {[
                ...
                { icon: <Clock className="w-3 h-3" />, text: isPreOrder ? `ETA: ${formatETA(product.eta_start!, product.eta_end)}` : "In Stock" },
              ].map(b => (
                <span key={b.text}
            ...
            <div style={{ borderTop: `1px solid ${BRAND.border}` }}>
              ...
              <div className="py-5 text-sm leading-relaxed" style={{ color: BRAND.muted }}>
                {activeTab === "details" && (
                  <p>{product.description || "Premium authentic sneakers from verified suppliers. Every pair is carefully inspected before shipping."}</p>
                )}
                {activeTab === "shipping" && (
